-- Migration: Add face_value column to tickets table and update ticket purchase/RSVP procedures

-- 1. Add face_value column to tickets table
ALTER TABLE public.tickets ADD COLUMN IF NOT EXISTS face_value integer;

-- 2. Backfill face_value for existing tickets (using ticket tier price, defaulting to amount_paid)
...
-- 3. Re-define finalize_purchase_after_payment with face_value population
...
BEGIN
  ...
  -- 7. Second Loop: Insert tickets with correct amount_paid and face_value per row
  v_discounted_counter := 0;
  FOR tier_line IN
    SELECT
      (el.value->>'tier_id')::uuid as tid,
...
-- 4. Re-define submit_rsvp with face_value population
...
BEGIN
  ...
  INSERT INTO public.tickets (
    ...
    v_ref,
    0,
    (v_tier.price::bigint * 100)::integer,
    1,
    'confirmed',
    v_code,
    true,
